...
export function normalizeProgressProduceCards(
  ...
  const normalized = rawCards.map((source, sourceIndex) => {
...
export function parseProgressProduceCardsJson(
  ...
  const cards = normalizeProgressProduceCards(extracted.cards, cardById, cardVariantByKey);
  if (!cards.length) throw new Error("Deleted除外後のproduceCardsが0枚です。");
  return { cards, path: extracted.path };
}
